...
def main():
    ...
    parser.add_argument("--freeze_layers", type=int, default=28)  # ALL 28 layers -- full backbone freeze (see NOTES.md option 3)
    ...
    for epoch in range(1, epochs + 1):
        ...
        torch.save({"model_state": model.state_dict(), "epoch": epoch, "val_acc": val_acc},
                   os.path.join(ckpt_dir, "exp4_latest.pt"))

        if val_acc > best_val:
            best_val = val_acc
            no_improve = 0
            torch.save({"model_state": model.state_dict(), "epoch": epoch, "val_acc": val_acc},
                       os.path.join(ckpt_dir, "exp4_best.pt"))
            print(f"  -> new best val_acc {val_acc:.4f}, saved exp4_best.pt", flush=True)
        else:
            no_improve += 1
    ...
    best_ckpt = torch.load(os.path.join(ckpt_dir, "exp4_best.pt"), map_location=device)
