...
BACKBONE = "intfloat/e5-large-v2"  # EXPERIMENT 1: only line changed vs iteration 4's model_v4.py
# (gte-large-en-v1.5 was tried first but its custom trust_remote_code RoPE
# implementation throws an out-of-bounds index error outside its expected
# code path -- not worth debugging someone else's custom modeling code.
# e5-large-v2 is a plain, standard BertModel: no custom code, still a
# modern (2023), purpose-built-for-embeddings checkpoint, well-ranked on
# MTEB -- same ablation question, without the risk.)
