...
BACKBONE = "Qwen/Qwen2.5-0.5B"  # EXPERIMENT 5: smaller decoder LLM, PARTIALLY UNFROZEN
# (experiment 4 found a 1.5B frozen decoder had the worst paraphrase gap
# in the whole project -- 22-32 points, vs 14 for full-fine-tuned encoder
# backbones. Leading hypothesis: freezing removed the adaptation capacity
# that the paraphrase fix (iteration 3) actually depends on -- the outcome
# encoder has to be ABLE to move for the augmentation trick to teach it
# invariance. Full fine-tuning the 1.5B model was ~5.4h/epoch, impractical
# on this hardware. Going 3x smaller makes partial fine-tuning (same
# freeze ratio used for the BERT-family backbones) affordable again --
# this isolates "frozen vs adaptable" using a decoder LLM, which is the
# open question, not a claim that smaller-is-better in general.
