...
class JEPAPolyEncoderV4(nn.Module):
    ...
    def compatibility_per_example(self, ctx_codes, outcome_emb_per_example):
        """EXPERIMENT 2 addition: each example in the batch has its OWN
        small candidate set (not one shared bank across the whole batch),
        matching a real Jev-style request where options are defined
        per-call. Same poly-encoder mechanism as `compatibility` above,
        just with the outcome side batched too.

        ctx_codes: (B, m, D) un-normalized poly codes.
        outcome_emb_per_example: (B, K, D) L2-normalized candidate vectors,
            K options specific to each of the B examples.
        Returns (B, K) compatibility logits.
        """
        ctx_codes = nn.functional.normalize(ctx_codes, dim=-1)  # (B, m, D)
        d = ctx_codes.size(-1)

        scores = torch.einsum("bmd,bkd->bmk", ctx_codes, outcome_emb_per_example) / (d ** 0.5)  # (B, m, K)
        weights = torch.softmax(scores, dim=1)

        final_ctx = torch.einsum("bmk,bmd->bkd", weights, ctx_codes)  # (B, K, D)
        final_ctx = nn.functional.normalize(final_ctx, dim=-1)

        compat = torch.einsum("bkd,bkd->bk", final_ctx, outcome_emb_per_example)  # (B, K)
        temp = self.log_temperature.exp().clamp(min=1e-3)
        return compat / temp
