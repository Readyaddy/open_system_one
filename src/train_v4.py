...
from model_v4 import JEPAPolyEncoderV4, get_tokenizer
...
def main():
    ...
    for epoch in range(1, epochs + 1):
        ...
        torch.save({"model_state": model.state_dict(), "epoch": epoch, "val_acc": val_acc},
                   os.path.join(ckpt_dir, "jepa_v4_latest.pt"))

        if val_acc > best_val:
            best_val = val_acc
            no_improve = 0
            torch.save({"model_state": model.state_dict(), "epoch": epoch, "val_acc": val_acc},
                       os.path.join(ckpt_dir, "jepa_v4_best.pt"))
            print(f"  -> new best val_acc {val_acc:.4f}, saved jepa_v4_best.pt", flush=True)
        else:
            no_improve += 1
    ...
    best_ckpt = torch.load(os.path.join(ckpt_dir, "jepa_v4_best.pt"), map_location=device)
